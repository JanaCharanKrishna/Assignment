...
def get_ai_client():
    """
    Groq-only client for OpenAI-family models hosted on Groq.
    """
    ...
    def is_placeholder(key: str) -> bool:
        return (not key) or ("your_key" in key) or ("gsk_" not in key) or (len(key) < 20)
    ...
    if not is_placeholder(groq_key):
        ...
        _provider = "GROQ"
        _model_name = settings.GROQ_MODEL or "openai/gpt-oss-20b"
        logger.info(f"AI subsystem online via {_provider} model={_model_name}")
        return _client, _provider, _model_name
...
def _build_interpretation_diagnostics(
    ...
    for curve in (dominant_curves or curves[:4]):
        ...
        high_zone = f"{round(min(high_depths), 1)}-{round(max(high_depths), 1)}" if high_depths else "n/a"
...
def _format_statistics(stats: dict) -> str:
    ...
    for curve, s in stats.items():
        if s.get("non_null_count", 0) > 0:
            lines.append(
                f"  {curve}: min={s.get('min')}, max={s.get('max')}, "
                f"mean={s.get('mean')}, points={s.get('non_null_count')}"
            )
        else:
...
def _format_sample_data(data: list[dict], curves: list[str], max_rows: int = 30) -> str:
    ...
    for row in sampled:
        ...
        lines.append(f"{row.get('depth')} | " + " | ".join(vals))
    return "\n".join(lines)


# -------------------------
# Groq call wrapper (retry)
# -------------------------
def _groq_chat_completion(
    ...
    retries = max(0, int(getattr(settings, "AI_MAX_RETRIES", 2)))
    timeout_s = max(5, int(getattr(settings, "AI_TIMEOUT_SECONDS", 45)))

    # Groq Python SDK supports max_completion_tokens (new style)
    # and may not accept timeout in all versions; if timeout unsupported, it's ignored by except branch.
    last_err = None
    for attempt in range(retries + 1):
        try:
            ...
            try:
                response = client.chat.completions.create(**kwargs, timeout=timeout_s)
            except TypeError:
    ...
    raise last_err if last_err else RuntimeError("Unknown Groq completion error")
...
def _fallback_interpretation(
    ...
    valid_curves = [curve for curve in curves if statistics.get(curve, {}).get("non_null_count", 0) > 0]

    if not valid_curves:
        return {
            "summary": f"No valid numeric samples were found in {depth_min}-{depth_max} for well '{well_name}'.",
            "geochemical_metrics": {
    ...
    hydro_candidates = [c for c in valid_curves if ("HC" in c.upper() or "GAS" in c.upper() or c.upper().startswith("C"))]
    ...
    zones = []
    for start_d, end_d in zone_bounds:
        ...
        zones.append({
            ...
            "key_markers": f"Relative hydrocarbon intensity={round(relative, 2)} using {', '.join(ranked_hydro[:3])}",
        })
...
def interpret_well_data(
    ...
    if not client:
        return _fallback_interpretation(well_name, curves, depth_min, depth_max, statistics, sample_data)

    stats_text = _format_statistics(statistics)
    sample_text = _format_sample_data(sample_data, curves, max_rows=30)
    diagnostics_text = _build_interpretation_diagnostics(statistics, sample_data, curves, depth_min, depth_max)
...
{{
  ...
  "gas_shows": [{{"depth_top": float, "depth_bottom": float, "analysis": "...", "fluid_probability": "High/Med/Low", "geological_context": "..."}}],
  "fluid_type": "...",
  "fluid_evidence": "...",
  "risk_profile": {{"seal_risk": "Low/Med/High", "saturation_risk": "Low/Med/High", "technical_summary": "..."}},
  "zones": [{{"depth_top": float, "depth_bottom": float, "characterization": "...", "key_markers": "..."}}],
  "recommendations": ["..."]
}}
...
"""
    ...
    try:
        content = _groq_chat_completion(
            ...
            model=model_name,
            messages=[
                {"role": "system", "content": "You are a precise geochemical analyst. Always return valid JSON object only."},
        ...
        logger.error(f"{provider} API error in interpret_well_data: {e}")
        return _fallback_interpretation(well_name, curves, depth_min, depth_max, statistics, sample_data)
...
def chat_with_data(
    ...
    if not client:
        return "AI chatbot is not available. Please configure GROQ_API_KEY in backend/.env."
...
Critical rules:
1) If the user asks for summary/overview ("summary", "summarize this", "tell me about this"), ALWAYS summarize available evidence.
   Never claim those words are missing entities.
2) If user asks "at 12800" or any number, treat it as a depth reference and infer nearest evidence window.
3) If a specific curve/depth is absent, say what's missing, then still provide best available nearby findings.
4) Do not hallucinate curves/depths not present in evidence.

Answer style:
- First line: "Key finding: <direct conclusion>"
- Then {profile['bullets']} concise bullets with concrete numbers (depth, min/max, mean, trend, correlation where available).
- End with one line: "Action: <specific next analysis/check>".
- Target response length: {profile['length']}.
...
"""
    ...
    try:
        content = _groq_chat_completion(
            ...
            model=model_name,
            messages=messages,
            temperature=0.45 + (normalized_detail * 0.03),
            max_tokens=900 + (normalized_detail * 280),
            json_mode=False,
        )
        return _clean_chat_text(content)

    except Exception as e:
        logger.error(f"{provider} Chat API error in chat_with_data: {e}")
        if "insufficient_quota" in str(e).lower():
            return "AI chat is unavailable because API quota is exhausted. Please check key/quota."
